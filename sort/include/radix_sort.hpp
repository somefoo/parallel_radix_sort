...
//WARNING Does not work under GCC 7.4.0 (cast to uint16_t is always 0).
template <typename Iterator, typename KeyGetter>
static inline void radix_sort_par_short(const Iterator begin, const Iterator end,
  ...
  //Start of actual work
  for (size_t depth = 0; depth < size_of_key/2; ++depth) {
    std::array<size_t, 65536> bucket_size{0};  // Init to 0
    // Read bytes and count occurances
    ...
    TIME_PRINT_RESET("Find bucket size");
		...
		//Prefix sum
		std::array<data_type*, 65536> bucket;
		bucket[0] = begin_cache;
    //same as std::array<data_type*, 65536> bucket{begin_cache}?
